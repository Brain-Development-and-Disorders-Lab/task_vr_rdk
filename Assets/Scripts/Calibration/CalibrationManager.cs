...
namespace Calibration
{
  public class CalibrationManager : MonoBehaviour
  {
    ...
    // Data storage
    private Dictionary<string, List<CalibrationVector>> GazeData = new() {
      {"E", new List<CalibrationVector>() },
      {"C", new List<CalibrationVector>() },
      {"NE", new List<CalibrationVector>() },
      {"N", new List<CalibrationVector>() },
      {"NW", new List<CalibrationVector>() },
      {"W", new List<CalibrationVector>() },
      {"SW", new List<CalibrationVector>() },
      {"S", new List<CalibrationVector>() },
      {"SE", new List<CalibrationVector>() },
    };

    // Calculated offset vectors
    private Dictionary<string, CalibrationVector> DirectionalOffsets = new();
    private CalibrationVector GlobalOffset;
    ...
    private void CalculateCalibrationValues()
    {
      ...
      foreach (string UnitVectorDirection in GazeData.Keys)
      {
        ...
        foreach (CalibrationVector VectorPair in GazeData[UnitVectorDirection])
        {
        ...
        DirectionalOffsets.Add(UnitVectorDirection, new CalibrationVector(VectorSum, VectorSum));
    ...
    public Dictionary<string, CalibrationVector> GetDirectionalOffsets()
    {
      return DirectionalOffsets;
    }

    public CalibrationVector GetGlobalOffset()
    {
      return GlobalOffset;
    }
